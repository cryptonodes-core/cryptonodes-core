...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::CNMC, nValue);
...
{
    std::string genesisblockhash = "00000b44d7b972caf926b5f57f35b7ad951286eca5fe7386e72bf64caee3b1e8";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (cryptonodes.conf).");
            QMessageBox::warning(this, "Cryptonodes Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
