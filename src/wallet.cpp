...
/**
 * Fees smaller than this (in ucnmc) are considered zero fee (for transaction creation)
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 CNMC.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-cnmc-address
                    CScript scriptChange;
...
{
    // We start paying the fees to stakers only after v1.4.0 fork.
    if (Params().NetworkID() == CBaseChainParams::MAIN && chainActive.Height() < SOFT_FORK_VERSION_140)
        nFees = 0;
