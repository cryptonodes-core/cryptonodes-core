...
/** Fees smaller than this (in ucnmc) are considered zero fee (for relaying and mining)
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
	...
	if (!tx.IsCoinBase() && pindexPrev->nHeight >= 250000) {
        if (pvChecks)
        ...
        if (!tx.IsCoinStake()) {
             if (nValueIn < tx.GetValueOut() && pindexPrev->nHeight >= 250000)
            ...
            if (nTxFee < 0 && pindexPrev->nHeight >= 250000)
                return state.DoS(100, error("CheckInputs() : %s nTxFee < 0", tx.GetHash().ToString()),
                    REJECT_INVALID, "bad-txns-fee-negative");
            nFees += nTxFee;
            if (!MoneyRange(nFees) && pindexPrev->nHeight >= 250000)
...
{
    RenameThread("cryptonodes-scriptch");
